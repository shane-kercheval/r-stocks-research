...
walk(df_top_variables$variable, ~{
    column_name = .
    temp_data <- data.frame(x_variable = df_stocks_full[, column_name], perc_change_stock_1year = df_stocks_full$perc_change_stock_1year)
    
    x_zoom_max <- quantile(temp_data$x_variable,  probs = c(0.90))
    x_zoom_min <- quantile(temp_data$x_variable,  probs = c(0.10))
	
    print(ggplot(data = temp_data, mapping = aes(x = x_variable, y = perc_change_stock_1year)) +
...
segmentation_dataset <- df_stocks_full %>% select(perc_change_stock_1year, diff_above_index_1year, dplyr::contains('cs_'))
dataset_scaled <- scale(segmentation_dataset)# %>% dplyr::select(-perc_change_stock_1year, -diff_above_index_1year))
...
if(pull_data)
{
	...
	saveRDS(ideal_number_of_clusters, file='./data/ideal_number_of_clusters_common_size.RDS')
	saveRDS(clusters, file='./data/clusters_common_size.RDS')
}else
{
	ideal_number_of_clusters <- readRDS('./data/ideal_number_of_clusters_common_size.RDS')
	clusters <- readRDS('./data/clusters_common_size.RDS')
}
...
```{r clustering_top_variables, echo=FALSE}
segmentation_dataset <- df_stocks_full[ , c('perc_change_stock_1year', 'diff_above_index_1year', df_top_variables$variable)]
dataset_scaled <- scale(segmentation_dataset)# %>% dplyr::select(-perc_change_stock_1year, -diff_above_index_1year))
...
if(pull_data)
{
	...
	saveRDS(ideal_number_of_clusters, file='./data/ideal_number_of_clusters_top_variables.RDS')
	saveRDS(clusters, file='./data/clusters_top_variables.RDS')
}else
{
	ideal_number_of_clusters <- readRDS('./data/ideal_number_of_clusters_top_variables.RDS')
	clusters <- readRDS('./data/clusters_top_variables.RDS')
}
...
```{r spot_check, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
seed <- 123

asdf <- map(df_stocks_datasets, ~ {
	spot_check_dataset <- .
	if('perc_change_stock_1year' %in% colnames(spot_check_dataset))
	{
		spot_check_dataset <- spot_check_dataset %>% rename(target_variable = perc_change_stock_1year)
	}
	if('diff_above_index_1year' %in% colnames(spot_check_dataset))
	{
		spot_check_dataset <- spot_check_dataset %>% rename(target_variable = diff_above_index_1year)
	}
	
	remove_cols <- nearZeroVar(spot_check_dataset, names = TRUE, freqCut = 2, uniqueCut = 20)
	...
	set.seed(seed)
	spot_check_folds <- createFolds(spot_check_dataset$target_variable, k = 5) # maintains the `churn` ratio
	...
	control <- trainControl(
		...
		index = spot_check_folds) # this ensures using the same fold for cross validation for each ML algorithm

	metric <- 'ROC'

	linear_pre_proccess <- c('knnImpute', 'nzv', 'center', 'scale') # `nzv` removes nearly constant columns
	impute_pre_process <- c('knnImpute', 'nzv')

	# Linear Regression
	set.seed(seed)
	fit.lm <- train(target_variable ~ . , data = spot_check_dataset, method = 'lm', metric=metric, preProc=linear_pre_proccess, trControl=control)
	# Neural Network - continuous/regression
	set.seed(seed)
	fit.neuralnet <- train(target_variable ~ ., data=spot_check_dataset, method='neuralnet', metric=metric, preProc=impute_pre_process, trControl=control)
	# Linear Discriminant Analysis
	set.seed(seed)
	fit.lda <- train(target_variable ~ ., data=spot_check_dataset, method='lda', metric=metric, preProc=linear_pre_proccess, trControl=control)
	# Logistic Regression
	set.seed(seed)
	fit.glm <- train(target_variable ~ ., data=spot_check_dataset, method='glm', metric=metric, preProc=linear_pre_proccess, trControl=control)
	#GLMNET
	set.seed(seed)
	fit.glmnet <- train(target_variable ~ ., data=spot_check_dataset, method='glmnet', metric=metric, preProc=linear_pre_proccess, trControl=control)
	# SVM Radial
	set.seed(seed)
	fit.svmRadial <- train(target_variable ~ ., data=spot_check_dataset, method='svmRadial', metric=metric, preProc=linear_pre_proccess, trControl=control, fit=FALSE)
	# kNN
	set.seed(seed)
	fit.knn <- train(target_variable ~ ., data=spot_check_dataset, method='knn', metric=metric, preProc=linear_pre_proccess, trControl=control)
	# Naive Bayes
	set.seed(seed)
	fit.nb <- train(target_variable ~ ., data=spot_check_dataset, method='nb', metric=metric, preProc=impute_pre_process, trControl=control)
	# CART
	set.seed(seed)
	fit.cart <- train(target_variable ~ ., data=spot_check_dataset, method='rpart', metric=metric, preProc=impute_pre_process, trControl=control)
	# C5.0
	set.seed(seed)
	fit.c50 <- train(target_variable ~ ., data=spot_check_dataset, method='C5.0', metric=metric, preProc=impute_pre_process, trControl=control)
	# Bagged CART
	set.seed(seed)
	fit.treebag <- train(target_variable ~ ., data=spot_check_dataset, method='treebag', metric=metric, preProc=impute_pre_process, trControl=control)
	# Random Forest
	set.seed(seed)
	fit.rf <- train(target_variable ~ ., data=spot_check_dataset, method='rf', metric=metric, preProc=impute_pre_process, trControl=control)
	# Stochastic Gradient Boosting (Generalized Boosted Modeling)
	set.seed(seed)
	fit.gbm <- train(target_variable ~ ., data=spot_check_dataset, method='gbm', metric=metric, preProc=impute_pre_process, trControl=control, verbose=FALSE)
...
NEED TO FLATTEN THIS LIST OR SOMETHING
resamples <- resamples(asdf)

# Table comparison
(resamples_summary <- summary(resamples))

# boxplot comparison
bwplot(resamples, scales = list(relation = 'free'), 
	   xlim = list( c( 	min(resamples_summary$statistics$ROC[,'Min.']), 
						max(resamples_summary$statistics$ROC[,'Max.'])), 
					c( 	min(resamples_summary$statistics$Sens[,'Min.']), 
						max(resamples_summary$statistics$Sens[,'Max.'])), 
					c( 	min(resamples_summary$statistics$Spec[,'Min.']), 
						max(resamples_summary$statistics$Spec[,'Max.']))))
