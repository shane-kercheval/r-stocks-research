...
stocks_data <- readRDS(file = './data/stocks_all_perc_change.RDS')
regression_dataset <- stocks_data %>%
	dplyr::rename(target = perc_change_stock_1year) %>%
...
recommended_columns_custom <- c('target', colnames(predictor_correlations))
...
if(refresh_models)
{
	...
	saveRDS(nlm_neur_net_basic, file = './regression_data/stocks_all_perc_change/nlm_neur_net_basic.RDS')
} else{
	nlm_neur_net_basic <- readRDS('./regression_data/stocks_all_perc_change/nlm_neur_net_basic.RDS')
}
...
if(refresh_models)
{
	...
	nlm_neur_net_basic_remove_corr <- train(target ~ ., data = regression_train[, recommended_columns_caret], method = 'nnet',
							...
							MaxNWts = parameter_neural_network_max_num_weights,
							maxit = parameter_neural_network_max_iterations)
	saveRDS(nlm_neur_net_basic_remove_corr, file = './regression_data/stocks_all_perc_change/nlm_neur_net_basic_remove_corr.RDS')
} else{
	nlm_neur_net_basic_remove_corr <- readRDS('./regression_data/stocks_all_perc_change/nlm_neur_net_basic_remove_corr.RDS')
}
...
if(refresh_models)
{
	...
	nlm_neur_net_averaging_remove_corr <- train(target ~ ., data = regression_train[, recommended_columns_caret], method = 'avNNet',
							...
							MaxNWts = parameter_neural_network_max_num_weights,
							maxit = parameter_neural_network_max_iterations)
	saveRDS(nlm_neur_net_averaging_remove_corr, file = './regression_data/stocks_all_perc_change/nlm_neur_net_averaging_remove_corr.RDS')
} else{
	nlm_neur_net_averaging_remove_corr <- readRDS('./regression_data/stocks_all_perc_change/nlm_neur_net_averaging_remove_corr.RDS')
}
...
if(refresh_models)
{
	...
	saveRDS(tree_m5, file = './regression_data/stocks_all_perc_change/tree_m5.RDS')
} else{
	tree_m5 <- readRDS('./regression_data/stocks_all_perc_change/tree_m5.RDS')
}
tree_m5$finalModel
...
if(refresh_models)
{
	...
	saveRDS(tree_m5rules, file = './regression_data/stocks_all_perc_change/tree_m5rules.RDS')
} else{
	tree_m5rules <- readRDS('./regression_data/stocks_all_perc_change/tree_m5rules.RDS')
}
...
if(refresh_models)
{
	...
	saveRDS(ensemble_random_forest_cond_inf, file = './regression_data/stocks_all_perc_change/ensemble_random_forest_cond_inf.RDS')
} else{
	ensemble_random_forest_cond_inf <- readRDS('./regression_data/stocks_all_perc_change/ensemble_random_forest_cond_inf.RDS')
}
...
model_stats <- map2(as.character(top_models$model), top_models$model_object, ~ {
	...
	if(is.null(model_object$preProcess$method))
	{
		...
		pre_process <- names(model_object$preProcess$method)[map_lgl(model_object$preProcess$method, ~ {length(.) > 0})]
		cat('\n\nPre-Processing:\n\n')
		cat(codebc(pre_process))
	}
	set.seed(seed)
	...
	model_plots(model = final_model, testing_set = regression_test)
})
